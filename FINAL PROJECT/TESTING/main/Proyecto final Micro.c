...
<body>\
\
<h1>ESP32 WEBSERVER - MALTINR</h1>\
\
<p>Testing the webserver on the esp32 :)</p>\
<h3> LED STATE : OFF </h3>\
\
\
<button class=\"button button1\" onclick= \"window.location.href='/ledon'\">LED ON</button>\
<button class=\"button button2\" onclick= \"window.location.href='/astable'\">ASTABLE</button>\
\
</body>\
</html>"
...
static const httpd_uri_t astable = {
    .uri       = "/astable",
    .method    = HTTP_GET,
    .handler   = astable_handler,
    /* Let's pass response string in user
     * context to demonstrate it's usage */
    .user_ctx  = "<!DOCTYPE html>\
<html>\
